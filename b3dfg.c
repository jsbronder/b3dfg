...
static void handle_cstate_change(unsigned long data)
{
	...
	/* Re-enable cable status interrupts */
	b3dfg_write32(fgdev, B3D_REG_HW_CTRL, 0x2);
}
...
static irqreturn_t b3dfg_intr(int irq, void *dev_id)
{
	...
	/* Check to see if this interrupt is for us.  The check for the revision
	 * is to workaround a firmware bug in earlier boards where cable status
	 * interrupts would be sent without update the DMA_STS register.
	 *
	 * TODO:  It would be nice to either drop this or find a better way to see
	 * if the interrupt was actually generated by the brontes board.
	 */
	if (unlikely(sts == 0 &&
			(fgdev->pdev->revision > 8 ||
			(fgdev->pdev->revision <= 8 && fgdev->transmission_enabled)))) {
		res = IRQ_NONE;
		goto out;
	}

	if (likely(fgdev->transmission_enabled)) {
	...
	if (unlikely(sts & 0x08 || (sts == 0 && fgdev->pdev->revision < 9))) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	struct b3dfg_dev *fgdev = pci_get_drvdata(pdev);
	return sprintf(buf, "%u\n",
		(b3dfg_read32(fgdev, B3D_REG_HW_CTRL) & 0x3) == 0x3 ? 1 : 0);
}
	...
	const struct pci_device_id *id)
{
	...
	/* Enable cable status interrupts */
	b3dfg_write32(fgdev, B3D_REG_HW_CTRL, 0x2);
