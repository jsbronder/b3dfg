...
\item Image capture of T3 begins. As B1 is locked, capture begins into B2. As B2 already contained a triplet, this triplet is discarded and SEG\_DROPPED is incremented.
...
\item T3 capture begins. B1 is still locked, so capture happens into B2. There was already a triplet in B2, so SEG\_DROPPED is incremented.
...
\item Driver receives interrupt, reads registers and observes that SEG\_DROPPED is non-zero (i.e. a frame has been lost).
