...
\begin{description}
%FIXME
\item[Vendor ID] We will need to purchase our own vendor ID before production from the PCI SIG. During development, we can use the Euraka ID. (2 bytes)
...
This ioctl shall be used to allocate a number of buffers in memory. Further invokations shall change the number of allocated buffers (by allocating more or freeing existing ones). This can only be called when transmission is disabled and will dequeue any buffers that have been queued.
